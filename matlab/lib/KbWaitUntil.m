function [secs, keyCode, deltaSecs] = KbWaitUntil(deviceNumber, forWhat, untilx, atleast)
...
if (forWhat == 2) | (forWhat == 3) %#ok<OR2>
    ...
    KbWaitUntil(deviceNumber, 1, untilx);
    ...
    if forWhat == 2
        ...
        % Wait for keypress:
        [secs, keyCode, deltaSecs] = KbWaitUntil(deviceNumber, 0, untilx);
...
while(1)
    ...
    if (isDown == ~forWhat && GetSecs >= t0 + atleast) || GetSecs >= (t0 + untilx)
        return;
    end
